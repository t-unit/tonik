...
void main() {
  ...
  test('nested JSON, enum, map, date, offset and explicit null', () async {
    ...
    expect(echoed.updated.timeZoneOffset, Duration.zero);
  });
  ...
  test(
    ...
    () async {
      ...
      expect(response.raw, contains('tags=a%2Cb'));
    },
  );
  ...
  test('oneOf card decoding and request roundtrip', () async {
    final response = success(await api.paymentOperation(kind: 'card'));
    expect(response.payment, isA<PaymentCard>());
  ...
  test('oneOf bank decoding', () async {
    final response = success(await api.paymentOperation(kind: 'bank'));
    expect(response.payment, isA<PaymentBank>());
    expect(response.toJson(), {
  ...
  test('multipart typed JSON part is parsed by Jackson', () async {
    final receipt = success(
      await api.jsonUploadOperation(
        body: JsonUpload(
          file: TonikFileBytes([0, 1, 127, 128, 255], fileName: 'sample.bin'),
          metadata: const Metadata(label: 'café + tea & milk', count: 2),
        ),
      ),
    );
    expect(receipt.text, 'café + tea & milk');
    expect(receipt.count, 2);
    expect(receipt.bytes, [0, 1, 127, 128, 255]);
    expect(receipt.fileName, 'sample.bin');
    expect(receipt.contentType, 'application/octet-stream');
    expect(receipt.raw.split(';').first, 'application/json');
  });
}
