...
void main() {
  group('ClassGenerator defaults', () {
    ...
    test(
      'required + default applies D3 — param is non-required, field is '
      'non-nullable',
      () {
        final model = ClassModel(
          isDeprecated: false,
    ...
    test(
      'nullable + default: null produces no static const (D14: absent vs '
      'explicit null collapses)',
      () {
        // Per D14, Property.defaultValue is Object? with no hasDefault flag,
        // so jsonValue == null is indistinguishable from "no default keyword".
        // The materialiser is short-circuited at the _resolveDefaults level
        ...
        expect(
          ...
          reason: 'raw default is null and Property.defaultValue is null — '
              'materialiser is short-circuited (D14)',
        );
...
''';
    ...
    test(
      'D6: alias-carried default propagates when property has no local '
      'default',
      () {
        final model = ClassModel(
          isDeprecated: false,
    ...
    test(
      'D6: property-local default overrides alias-carried default',
      () {
        final model = ClassModel(
          isDeprecated: false,
