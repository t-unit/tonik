...
''';
  ...
  _$formData.files.add(
    ...
    test('NeverModel property name with dollar sign emits raw literal', () {
      final content = multipartContentFixture(testContext, [
        multipartPartFixture(
          name: r'$total',
          model: NeverModel(context: testContext, isNullable: false),
            ...
            switch (body.avatar) {
              case TonikFileBytes(:final bytes, :final fileName):
                _$formData.files.add(MapEntry(
                  ...
                  MultipartFile.fromBytes(bytes, filename: (fileName ?? r'avatar').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                ));
              case TonikFilePath(:final path, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'avatar').replaceAll(r'\', r'\\'),
                  await MultipartFile.fromFile(path, filename: (fileName ?? r'avatar').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                ));
            }
            return _$formData;
          }
              ...
              switch (body.document!) {
                case TonikFileBytes(:final bytes, :final fileName):
                  _$formData.files.add(MapEntry(
                    ...
                    MultipartFile.fromBytes(bytes, filename: (fileName ?? r'document').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
                case TonikFilePath(:final path, :final fileName):
                  _$formData.files.add(MapEntry(
                    (r'document').replaceAll(r'\', r'\\'),
                    await MultipartFile.fromFile(path, filename: (fileName ?? r'document').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
              }
            }
            return _$formData;
              ...
              switch (body.photo!) {
                case TonikFileBytes(:final bytes, :final fileName):
                  _$formData.files.add(MapEntry(
                    ...
                    MultipartFile.fromBytes(bytes, filename: (fileName ?? r'photo').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
                case TonikFilePath(:final path, :final fileName):
                  _$formData.files.add(MapEntry(
                    (r'photo').replaceAll(r'\', r'\\'),
                    await MultipartFile.fromFile(path, filename: (fileName ?? r'photo').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
              }
            }
            return _$formData;
            ...
            switch (body.file) {
              case TonikFileBytes(:final bytes, :final fileName):
                _$formData.files.add(MapEntry(
                  ...
                  MultipartFile.fromBytes(bytes, filename: (fileName ?? r'file').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                ));
              case TonikFilePath(:final path, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'file').replaceAll(r'\', r'\\'),
                  await MultipartFile.fromFile(path, filename: (fileName ?? r'file').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                ));
            }
            return _$formData;
          }
  ...
  group('complex object properties', () {
    ...
    test('generates deepObject-encoded file part for AllOfModel property', () {
      final allOfModel = AllOfModel(
        name: 'CombinedAddress',
        isDeprecated: false,
        models: [StringModel(context: testContext)],
    ...
    test('generates deepObject-encoded file part for OneOfModel property', () {
      final oneOfModel = OneOfModel(
        name: 'AddressVariant',
        isDeprecated: false,
        models: [
    ...
    test('generates deepObject-encoded file part for AnyOfModel property', () {
      final anyOfModel = AnyOfModel(
        name: 'AddressMixed',
        isDeprecated: false,
        models: [
    ...
    group('content-based mode (URL-encoded)', () {
      ...
      test('generates URL-encoded file part for AllOfModel property', () {
        final allOfModel = AllOfModel(
          name: 'CombinedAddress',
          isDeprecated: false,
          models: [StringModel(context: testContext)],
      ...
      test('URL-encoded ClassModel property with backslash in rawName escapes '
          'rawName in the part name literal', () {
        final innerClass = ClassModel(
          name: 'PathTo',
          isDeprecated: false,
      ...
      test('style-based raw parts pass per-part headers to every part', () {
        final innerClass = ClassModel(
          name: 'Address',
          isDeprecated: false,
          properties: [],
      ...
      test('non-form styled object parts throw instead of rendering with '
          'form semantics', () {
        final innerClass = ClassModel(
          name: 'Address',
          isDeprecated: false,
      ...
      test('non-exploded style-based mode emits one raw comma-joined part', () {
        final innerClass = ClassModel(
          name: 'Address',
          isDeprecated: false,
          properties: [],
      ...
      test('URL-encoded property with per-part headers passes headers to '
          'MultipartFile', () {
        final innerClass = ClassModel(
          name: 'Address',
          isDeprecated: false,
  ...
  group('MapModel properties', () {
    test('generates JSON-encoded file part for required MapModel property', () {
    ...
    test('generates JSON-encoded file part for optional MapModel property', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
        examples: const [],
    ...
    test(
      ...
      () {
        ...
        final content = multipartContentFixture(testContext, [
          multipartPartFixture(name: 'metadata', model: mapModel),
        ]);
    ...
    test('generates deepObject error for MapModel property', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
        examples: const [],
    ...
    test('deepObject error for MapModel uses raw literal when property name '
    ...
    test('URL-encoded map dynamic values use typed form encoding', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
        examples: const [],
    ...
    test('generates URL-encoded file part for optional MapModel property', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
        examples: const [],
    ...
    test('URL-encoded MapModel property with single quote in rawName escapes '
    ...
    test('URL-encoded MapModel property with backslash in rawName escapes '
        'rawName in the EncodingException literal', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
    ...
    test('URL-encoded MapModel property with dollar sign in rawName escapes '
        'rawName in the EncodingException literal', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
    ...
    test('generates JSON-encoded file part for MapModel through alias', () {
      final mapModel = MapModel(
        valueModel: StringModel(context: testContext),
        context: testContext,
        examples: const [],
  ...
  group('array properties', () {
    ...
    test('deepObject error for list uses raw literal when property name '
        'has special characters', () {
      final content = multipartContentFixture(testContext, [
        multipartPartFixture(
          name: "it's-tags",
        ...
        collapseWhitespace(
          ...
          void test() {
            ...
            for (final item in body.files) {
              switch (item) {
                case TonikFileBytes(:final bytes, :final fileName):
                  _$formData.files.add(MapEntry((r'files').replaceAll(r'\', r'\\'), MultipartFile.fromBytes(bytes, filename: (fileName ?? r'files').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'))));
                case TonikFilePath(:final path, :final fileName):
                  _$formData.files.add(MapEntry((r'files').replaceAll(r'\', r'\\'), await MultipartFile.fromFile(path, filename: (fileName ?? r'files').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'))));
              }
            }
            return _$formData;
          }
    ...
    group('content-based mode (no style fields)', () {
      ...
      test('list of strings, text/plain contentType (parser default) '
      ...
      test('list of BinaryModel, content-based mode (no style) → '
      ...
      test('list of ListModel items (array-of-arrays), '
      ...
      test('arrays-of-arrays error uses raw literal when property name '
          'has special characters', () {
        final content = multipartContentFixture(testContext, [
          multipartPartFixture(
            name: r'$matrix',
      ...
      test('list with explicit unsupported contentType, '
          'content-based mode → EncodingException', () {
        final content = multipartContentFixture(testContext, [
          multipartPartFixture(
            name: 'items',
      ...
      test('unsupported contentType error uses raw literal when property name '
          'has special characters', () {
        final content = multipartContentFixture(testContext, [
          multipartPartFixture(
            name: "it's-items",
  ...
  group('per-part headers', () {
          ...
          void test() {
            ...
            switch (body.file) {
              case TonikFileBytes(:final bytes, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'file').replaceAll(r'\', r'\\'),
                  MultipartFile.fromBytes(bytes, filename: (fileName ?? r'file').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$fileHeaders),
                ));
              case TonikFilePath(:final path, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'file').replaceAll(r'\', r'\\'),
                  await MultipartFile.fromFile(path, filename: (fileName ?? r'file').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$fileHeaders),
                ));
            }
            return _$formData;
          }
    ...
    test('non-exploded space-delimited string list with headers wraps the '
        ...
        collapseWhitespace(
          ...
          void test() {
            ...
            for (final item in body.files) {
              switch (item) {
                case TonikFileBytes(:final bytes, :final fileName):
                  _$formData.files.add(MapEntry((r'files').replaceAll(r'\', r'\\'), MultipartFile.fromBytes(bytes, filename: (fileName ?? r'files').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$filesHeaders)));
                case TonikFilePath(:final path, :final fileName):
                  _$formData.files.add(MapEntry((r'files').replaceAll(r'\', r'\\'), await MultipartFile.fromFile(path, filename: (fileName ?? r'files').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$filesHeaders)));
              }
            }
            return _$formData;
          }
    ...
    test('primitive field with json contentType and headers uses jsonEncode in '
        'MultipartFile.fromString', () {
      final content = multipartContentFixture(testContext, [
        multipartPartFixture(
          name: 'count',
          ...
          void test() {
            ...
            switch (body.document) {
              case TonikFileBytes(:final bytes, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'document').replaceAll(r'\', r'\\'),
                  MultipartFile.fromBytes(bytes, filename: (fileName ?? r'document').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$documentHeaders),
                ));
              case TonikFilePath(:final path, :final fileName):
                _$formData.files.add(MapEntry(
                  (r'document').replaceAll(r'\', r'\\'),
                  await MultipartFile.fromFile(path, filename: (fileName ?? r'document').replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream'), headers: _$documentHeaders),
                ));
            }
            return _$formData;
          }
  ...
  group('special characters in multipart field names and content types', () {
    test('generates valid code when rawContentType contains single quote', () {
      final content = multipartContentFixture(testContext, [
        multipartPartFixture(
          name: 'name',
          model: StringModel(context: testContext),
    ...
    test('generates valid code when field name contains single quote', () {
      final content = multipartContentFixture(testContext, [
        multipartPartFixture(
          name: "it's-field",
          model: BinaryModel(context: testContext),
              ...
              switch (body.itsField) {
                case TonikFileBytes(:final bytes, :final fileName):
                  _$formData.files.add(MapEntry(
                    ...
                    MultipartFile.fromBytes(bytes, filename: (fileName ?? r"it's-field").replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
                case TonikFilePath(:final path, :final fileName):
                  _$formData.files.add(MapEntry(
                    (r"it's-field").replaceAll(r'\', r'\\'),
                    await MultipartFile.fromFile(path, filename: (fileName ?? r"it's-field").replaceAll(r'\', r'\\'), contentType: DioMediaType.parse(r'application/octet-stream')),
                  ));
              }
              return _$formData;
            }
