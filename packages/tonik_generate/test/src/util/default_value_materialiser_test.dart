...
void main() {
  ...
  group('materialiseConstDefault — enums', () {
    ...
    test('value matches fallbackValue but is NOT in values — returns null '
    ...
    test('matched variant name is stable when fallbackValue nameOverride '
    ...
    test('value present in both values AND fallbackValue resolves to the '
    ...
    test('nullable enum with a value in the enum returns null because const '
  ...
  group('materialiseConstDefault — collections', () {
    ListModel listOf(Model content) =>
        ListModel(content: content, context: context, examples: const []);

    MapModel mapOf(Model valueModel) =>
        MapModel(valueModel: valueModel, context: context, examples: const []);
    ...
    test('MapModel<IntegerModel> + map of ints yields const map', () {
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(formatBody("const <String, int>{r'x': 1, r'y': 2}")),
      );
    });
    ...
    test('useImmutableCollections: true wraps ListModel<StringModel> in '
        'IListConst', () {
      final result = materialiseConstDefault(
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(formatBody("const IListConst(<String>[r'a'])")),
      );
    });

    test('useImmutableCollections: true wraps MapModel<IntegerModel> in '
        'IMapConst', () {
      final result = materialiseConstDefault(
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(
          formatBody("const IMapConst(<String, int>{r'a': 1})"),
        ),
      );
    });
  ...
  group('materialiseConstDefault — AnyModel', () {
    ...
    test('AnyModel + list of primitives yields const list<Object?>', () {
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(formatBody("const <Object?>[1, true, r'x']")),
      );
    });
    ...
    test('AnyModel + list containing null yields const list with null', () {
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(formatBody("const <Object?>[1, null, r'x']")),
      );
    });
    ...
    test('AnyModel + map of primitives yields const map<String, Object?>', () {
      ...
      expect(
        collapseWhitespace(renderExpression(result!)),
        collapseWhitespace(
          formatBody("const <String, Object?>{r'a': 1, r'b': true}"),
        ),
      );
    });
