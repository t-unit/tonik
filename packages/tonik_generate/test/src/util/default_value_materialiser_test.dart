...
void main() {
  ...
  group('materialiseConstDefault — null handling (D14 collapse)', () {
    // Per D14, jsonValue == null is treated as "no default keyword present"
    // regardless of target nullability. The materialiser returns null and
    // callers follow the no-default path. There is no observable behaviour
    // difference between absent and explicit-null defaults at the carrier
    // level (Property.defaultValue is Object? with no hasDefault flag).
    ...
    test('null + alias-wrapped nullable primitive returns null (D14 collapse)',
        () {
  ...
  group('materialiseConstDefault — non-primitive targets return null', () {
    test('ClassModel returns null (Strategy B target)', () {
      final result = materialiseConstDefault(
    ...
    test('DateTimeModel returns null (non-const leaf)', () {
      final result = materialiseConstDefault(
    ...
    test('AnyModel returns null (M1 scope only covers primitives)', () {
      final result = materialiseConstDefault(
        jsonValue: 'anything',
