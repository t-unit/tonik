...
class RequestBodyImporter {
  ...
  core.MultipartRequestContent _importMultipartContent(
    ...
    return core.MultipartRequestContent(
      model: schema == null
          ? core.AnyModel(context: context.push('body'))
          : modelImporter.importSchema(schema, context.push('body')),
      encoding: {
        for (final entry
            in (mediaType.encoding ?? <String, Encoding>{}).entries)
          entry.key: _importPartEncoding(entry.value, entry.key, context),
      },
      rawContentType: rawContentType,
      examples: exampleImporter.fromMediaType(mediaType),
    );
  ...
  core.PartEncoding _importPartEncoding(
    ...
    final useStyleMode =
        !openApiObject.openapi.startsWith('3.0') &&
        (encoding.style != null ||
    ...
    final style = useStyleMode
        ? (_mapSerializationStyle(encoding.style) ?? core.EncodingStyle.form)
        : null;
    final textEncoding = rawContentType == null
        ? null
        : _resolveTextEncoding(
            rawContentType,
            applicable: !useStyleMode && contentType != core.ContentType.bytes,
          );
    return core.PartEncoding(
      ...
      headers: _importEncodingHeaders(encoding, propertyName, context),
      style: style,
      explode: useStyleMode
          ? (encoding.explode ?? (style == core.EncodingStyle.form))
          : null,
